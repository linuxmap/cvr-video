...
#define PU_CONTRAST_MAX_VAL         255
...
{
    static struct v4l2_buffer ubuf;
    struct v4l2_buffer vbuf;
    unsigned int i;
    int ret;
...
uvc_events_process_data(struct uvc_device *dev, struct uvc_request_data *data)
{
    ...
    default:
        ...
        printf("g_cs: %u, g_entity_id: %u\n", g_cs, g_entity_id);
        ret = uvc_events_process_control_data(dev, g_cs, g_entity_id, data);
        if (ret < 0)
            goto err;
    ...
    if (dev->control == UVC_VS_COMMIT_CONTROL) {
        if (uvc_buffer_process_state())
            return 0;
        video_record_set_fps(10000000 / target->dwFrameInterval);
        dev->fcc = format->fcc;
        ...
        uvc_set_user_fcc(fmt.fmt.pix.pixelformat);
        if (uvc_buffer_init())
            goto err;
...
uvc_gadget_main()
{
    ...
    snprintf(uvc_devname, sizeof(uvc_devname), "/dev/video%d", uvc_get_user_video_id());
